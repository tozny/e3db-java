...
* [4.0.0-RC2](https://tozny.github.io/e3db-java/docs/4.0.0-RC2/) - The most recently released version of the client.
...
new RegistrationImpl().register(token, "client1", host, new ResultHandler<Config>() {
  @Override
...
new RegistrationImpl().register(token, "client1", host, pinner, new ResultHandler<Config>() {
  @Override
  public void handle(Result<Config> r) {
    if(! r.isError()) {
      Config info = r.asValue();
